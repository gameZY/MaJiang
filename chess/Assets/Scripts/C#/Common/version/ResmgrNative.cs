...
public class ResmgrNative
{
    ...
    public void Update()
    {
        ...
        foreach (var r in runnner)
        {
            if (r.www.isDone)
            {
                downloadbytes += r.www.bytesDownloaded;
                Debug.Log("======down task runner====" + downloadbytes.ToString());
    ...
    class DownTask
    {
        ...
        public DownTask(string remotepath, string localpath, string tag, Action<WWW, string> onload)
        {
    ...
    class DownTaskRunner
    {
        public DownTaskRunner(DownTask task)
        {
            ...
            else
            {
                ...
                if (lastindex == -1)
                {
                    form.AddField("path", "null");
                    form.AddField("file", localpath);
                }
                else
                {
                    string path = localpath.Substring(0, lastindex+1);
                    string file = localpath.Substring(lastindex+1, localpath.Length - lastindex - 1);
                    form.AddField("path", path);
                    form.AddField("file", file);
                }
                string remotepath = this.task.remotepath;
                remotepath = remotepath.Substring(0, remotepath.Length - 1);
                www = new WWW(remotepath,form);
            }
        }
        public WWW www;
    ...
    public void LoadFromRemote(string path, string tag, Action<WWW, string> onLoad)
    {
        ...
        LoadRemote(remoteurl, path, tag, onLoad);
    }
    ...
    void LoadRemote(string remotepath,string localpath,string tag,Action<WWW, string> onLoad)
    {
        task.Enqueue(new DownTask(remotepath,localpath,tag, onLoad));
        taskState.taskcount++;
    }
