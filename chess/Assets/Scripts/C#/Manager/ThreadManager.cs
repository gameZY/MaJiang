...
namespace Teacher.Manager {
    ...
    public class ThreadManager : View {
        ...
        LuaInterface.LuaFunction UpdateProgress = null;
        LuaInterface.LuaFunction DownLoadFinish = null;
        bool is_updateprogress = false;
        bool is_finish = false;
        float progress_str = 0.0f;
        byte[] datas = null;
        ...
        void Update()
        {
            if (is_updateprogress)
            {
                if (UpdateProgress != null)
                {
                    UpdateProgress.Call(progress_str);

                    if (is_finish)
                    {
                        is_updateprogress = false;
                        is_finish = false;
                        if (DownLoadFinish != null)
                        {
                            DownLoadFinish.Call(datas);
                            datas = null;
                        }
                    }
                }
        ...
        {
            string url = evParams[1].ToString();
            bytesReceived = 0;

            UpdateProgress = _UpdateProgress;
            DownLoadFinish = _DownLoadFinish;
        ...
        private void ProgressChange(object sender, DownloadProgressChangedEventArgs e)
        {
            ...
            float percent = (float)bytesReceived / e.TotalBytesToReceive;
