...
namespace Teacher.Manager {
    ...
    public class ResourceManager : View {
        ...
        public void LoadImage(string abname, string assetname,GameObject obj,LuaFunction func)
        {
            abname = abname.ToLower();
            StartCoroutine(OnLoadImage(abname,assetname,obj,func));
        }

        IEnumerator OnLoadImage(string abname, string assetName,GameObject obj,LuaFunction func)
        {
            // Load asset from assetBundle.
            string abName = abname.ToLower() + AppConst.ExtName;
            AssetBundleAssetOperation request = ResourceManager.LoadAssetAsync(abName, assetName, typeof(Sprite));
            if (request == null) yield break;
            yield return StartCoroutine(request);
            ...
            if (func != null)
            {
                func.Call(prefab,obj);
                func.Dispose();
                func = null;
            }
        }

        public void LoadMtr(string abname, string assetname, GameObject obj, LuaFunction func)
        {
            abname = abname.ToLower();
            StartCoroutine(OnLoadMtr(abname, assetname, obj, func));
        }

        IEnumerator OnLoadMtr(string abname, string assetName, GameObject obj, LuaFunction func)
        {
            // Load asset from assetBundle.
            string abName = abname.ToLower() + AppConst.ExtName;
            AssetBundleAssetOperation request = ResourceManager.LoadAssetAsync(abName, assetName, typeof(Material));
            if (request == null) yield break;
            yield return StartCoroutine(request);
