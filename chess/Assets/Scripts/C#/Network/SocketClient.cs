...
public class SocketClient {
    ...
    void OnConnect(IAsyncResult asr) {
        ...
        {
            OnDisconnected(DisType.Disconnect, e.Message);
        }
    }
    ...
    void WriteMessage(byte[] message) {
        ...
        using (ms = new MemoryStream()) {
            ...
            writer.Write(msglen - 6);

            ByteBuffer buffer = new ByteBuffer(message);
            int msgid = buffer.ReadInt();
            writer.Write(msgid);
            
            byte[] msg = new byte[msglen - 6];
    ...
    void OnReceive(byte[] bytes, int length) {
        ...
        while (RemainingBytes() > size) {
            ...
            if (RemainingBytes() >= messageLen) {
                ...
                OnReceivedMessage(ms,messageLen);
            } else {
                //Back up the position two bytes
                memStream.Position = memStream.Position - size;
                break;
            }
        }
    ...
    /// </summary>
    /// <param name="ms"></param>
    void OnReceivedMessage(MemoryStream ms,int length) {
        BinaryReader r = new BinaryReader(ms);
        byte[] message = r.ReadBytes((int)(ms.Length - ms.Position));
        //int msglen = message.Length;

        ByteBuffer buffer = new ByteBuffer(message);
        buffer.SetBufferLength(length);
